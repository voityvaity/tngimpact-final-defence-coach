...
function hasSessionWork(){return state.questions.length>0||Object.keys(state.records).length>0||$('topic').value.trim()||$('context').value.trim()||$('answer').value.trim()}
...
function restore(){
 ...
 }catch(_){state.lastLanguage=lang()}
}
...
function fillExample(){const ex=examples[lang()]||examples.en;$('topic').value=ex.topic;$('context').value=ex.context;updateCounts();persistRaw()}
async function guidedDemo(){
 if(hasSessionWork()&&!state.questions.length&&!confirm(t().replaceContextConfirm))return;
 clearPanel(false);fillExample();await generatePanel(true);
 const item=state.questions[0];if(item){$('answer').value=(examples[lang()]||examples.en).answer;saveCurrentAnswer();updateCounts();$('answerCard').scrollIntoView({behavior:'smooth',block:'center'})}
}
...
async function generatePanel(fromDemo=false){
 ...
 if(state.questions.length&&Object.keys(state.records).length&&!fromDemo&&!confirm(t().resetConfirm))return;
 ...
 try{
  ...
  state.questions=data.questions||[];state.currentIndex=0;state.records={};
  ...
  $('answer').value='';renderQuestions();renderSelected();updateProgress();persistRaw();
...
function selectQuestion(index){
 ...
 const item=state.questions[index],rec=state.records[item.id]||{};$('answer').value=rec.answer||'';
 $('evaluateError').textContent='';$('voiceStatus').textContent='';renderQuestions();renderSelected();updateCounts();
 if(rec.result){showResult(rec.result);$('resultCard').classList.remove('hidden')}else{$('resultCard').classList.add('hidden')}
 persistRaw();$('answerCard').scrollIntoView({behavior:'smooth',block:'center'});
}
function renderSelected(){
 const item=state.questions[state.currentIndex];if(!item)return;
 $('selectedAvatar').textContent=state.currentIndex+1;$('selectedRole').textContent=item.role||'';$('selectedQuestion').textContent=item.question||'';
}
function renderExistingResult(){
 const item=state.questions[state.currentIndex];const result=item&&state.records[item.id]?.result;
 if(result){showResult(result);$('resultCard').classList.remove('hidden')}
}
...
async function evaluateAnswer(){
 ...
 try{
  ...
  state.records[item.id]={answer,result:data};showResult(data);renderQuestions();updateProgress();persistRaw();
...
function showResult(data){
 ...
 const keys=['clarity','relevance','evidence','structure'];
 $('breakdown').innerHTML=keys.map(key=>{const value=Number(data.dimensions?.[key]||0);return `<div class="metric-row"><span>${escapeHtml(t()[key])}</span><div class="bar"><span style="width:${value}%"></span></div><b>${value}</b></div>`}).join('');
 $('strengths').innerHTML=(data.strengths||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join('');$('improvements').innerHTML=(data.improvements||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join('');
 $('feedback').textContent=data.feedback||'';$('frameworkText').textContent=data.improved_answer||'';$('nextTip').textContent=data.next_tip||'';
 $('scoreExplanation').textContent=(data.mode==='llm')?t().aboutScoreLive:t().aboutScoreDemo;
 if(data.notice){$('resultNotice').textContent=data.notice;$('resultNotice').classList.remove('hidden')}else{$('resultNotice').classList.add('hidden')}
}

function updateProgress(){
 const done=state.questions.filter(q=>state.records[q.id]?.result).length,total=state.questions.length||5,percent=Math.round(done/total*100);
 $('progressBar').style.width=`${percent}%`;$('topProgressBar').style.width=`${percent}%`;
 ...
 const entries=state.questions.map(q=>state.records[q.id]?.result).filter(Boolean);
 if(!entries.length){$('averageScore').textContent='—';$('focusArea').textContent='—';$('sessionComplete').classList.add('hidden');return}
 $('averageScore').textContent=Math.round(entries.reduce((sum,r)=>sum+Number(r.score||0),0)/entries.length);
 const totals={clarity:0,relevance:0,evidence:0,structure:0};entries.forEach(r=>Object.keys(totals).forEach(k=>totals[k]+=Number(r.dimensions?.[k]||0)));
 const weakest=Object.keys(totals).sort((a,b)=>totals[a]-totals[b])[0];$('focusArea').textContent=t()[weakest];
 if(done===total&&state.questions.length){$('sessionComplete').classList.remove('hidden')}else{$('sessionComplete').classList.add('hidden')}
}
function weakestQuestionIndex(){
 let best={index:0,value:Infinity};state.questions.forEach((q,index)=>{const r=state.records[q.id]?.result;if(!r)return;const avg=Object.values(r.dimensions||{}).reduce((a,b)=>a+Number(b||0),0)/4;if(avg<best.value)best={index,value:avg}});return best.index
}
function nextQuestion(){
 if(!state.questions.length)return;let target=state.questions.findIndex((q,i)=>i!==state.currentIndex&&!state.records[q.id]?.result);if(target<0)target=(state.currentIndex+1)%state.questions.length;selectQuestion(target)
}
function retakeWeakest(){selectQuestion(weakestQuestionIndex());$('answer').focus()}

function clearPanel(clearResearch=true){
 state.questions=[];state.currentIndex=0;state.records={};$('answer').value='';$('questionsCard').classList.add('hidden');$('answerCard').classList.add('hidden');$('resultCard').classList.add('hidden');$('sessionComplete').classList.add('hidden');
 if(clearResearch){$('topic').value='';$('context').value='';$('fileName').textContent=''}
...
async function uploadResearch(file){
 if(!file)return;if(file.size>6*1024*1024){$('fileStatus').textContent=t().requestFailed;return}
 ...
 try{
  ...
  if(!response.ok)throw new Error(data.detail||t().requestFailed);
  $('context').value=data.text||'';if(!$('topic').value.trim()&&data.suggested_topic)$('topic').value=data.suggested_topic;
  $('fileName').textContent=data.filename||file.name;$('fileStatus').textContent=`${t().fileLoaded} ${data.filename}.${data.truncated?' '+t().fileTruncated:''}`;updateCounts();persistRaw();
 }catch(err){$('fileStatus').textContent=err.message||t().requestFailed}finally{$('fileInput').value=''}
}

function hearQuestion(){
 const item=state.questions[state.currentIndex];if(!item||!('speechSynthesis'in window)){$('voiceStatus').textContent=t().voiceUnavailable;return}
 speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(item.question);utterance.lang=speechLanguages[lang()]||'en-US';utterance.rate=.96;speechSynthesis.speak(utterance);$('voiceStatus').textContent=t().hearing
}
function setupVoice(){
 const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;
 if(!Recognition){$('dictateBtn').disabled=true;$('dictateBtn').title=t().voiceUnavailable;return}
 recognition=new Recognition();recognition.interimResults=false;recognition.continuous=false;
 recognition.onstart=()=>{$('voiceStatus').textContent=t().listening;$('dictateBtn').disabled=true};recognition.onend=()=>{$('dictateBtn').disabled=false};recognition.onerror=()=>{$('voiceStatus').textContent=t().voiceUnavailable;$('dictateBtn').disabled=false};
 recognition.onresult=e=>{const text=Array.from(e.results).map(r=>r[0].transcript).join(' ').trim();$('answer').value=[$('answer').value.trim(),text].filter(Boolean).join(' ');updateCounts();saveCurrentAnswer()}
}
...
function updateMode(){
 const live=state.health?.mode==='llm';$('engineBadge').textContent=live?t().liveAI:t().demoSimulation;$('engineBadge').classList.toggle('mode-live',live);$('engineBadge').classList.toggle('mode-demo',!live)
}
...
async function copyFramework(){
 const text=$('frameworkText').textContent;try{if(navigator.clipboard)await navigator.clipboard.writeText(text);else{const ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove()}$('copyStatus').textContent=t().copied}catch(_){$('copyStatus').textContent=t().requestFailed}
}

function handleLanguageChange(){
 const next=lang();if(next===state.lastLanguage)return;
 if((state.questions.length||Object.keys(state.records).length)&&!confirm(trFor(state.lastLanguage).languageChangeConfirm)){$('language').value=state.lastLanguage;return}
 if(state.questions.length||Object.keys(state.records).length)clearPanel(false);
 state.lastLanguage=next;applyLanguage();persistRaw();setupVoice()
}
function restoreVisibleState(){
 if(!state.questions.length)return;
 $('questionsCard').classList.remove('hidden');$('answerCard').classList.remove('hidden');renderQuestions();renderSelected();
 const item=state.questions[state.currentIndex];$('answer').value=state.records[item?.id]?.answer||'';renderExistingResult();updateCounts();updateProgress()
}

$('language').addEventListener('change',handleLanguageChange);$('guidedDemoBtn').addEventListener('click',guidedDemo);$('ownBtn').addEventListener('click',()=>$('researchCard').scrollIntoView({behavior:'smooth'}));$('exampleBtn').addEventListener('click',fillExample);$('generateBtn').addEventListener('click',()=>generatePanel(false));$('fileInput').addEventListener('change',e=>uploadResearch(e.target.files?.[0]));$('dropzone').addEventListener('click',e=>{if(e.target.id!=='fileInput')$('fileInput').click()});$('dropzone').addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();$('fileInput').click()}});['dragenter','dragover'].forEach(ev=>$('dropzone').addEventListener(ev,e=>{e.preventDefault();$('dropzone').classList.add('dragover')}));['dragleave','drop'].forEach(ev=>$('dropzone').addEventListener(ev,e=>{e.preventDefault();$('dropzone').classList.remove('dragover')}));$('dropzone').addEventListener('drop',e=>uploadResearch(e.dataTransfer.files?.[0]));$('sampleAnswerBtn').addEventListener('click',()=>{$('answer').value=(examples[lang()]||examples.en).answer;updateCounts();saveCurrentAnswer()});$('hearBtn').addEventListener('click',hearQuestion);$('dictateBtn').addEventListener('click',startDictation);$('evaluateBtn').addEventListener('click',evaluateAnswer);$('copyBtn').addEventListener('click',copyFramework);$('nextBtn').addEventListener('click',nextQuestion);$('resetBtn').addEventListener('click',resetSession);$('retakeBtn').addEventListener('click',retakeWeakest);$('topic').addEventListener('input',persistRaw);$('context').addEventListener('input',updateCounts);$('answer').addEventListener('input',updateCounts);
